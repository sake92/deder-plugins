...
class ProtobufGenerationTasksSuite extends FunSuite {
  ...
  private val stubCoreTasks = new CoreTasksApi {
    ...
    override val classesTask: AbstractTask[os.Path] = constantTask("classes", os.root)
    override val allClassesDirsTask: AbstractTask[Seq[os.Path]] = constantTask("allClassesDirs", Seq.empty)
