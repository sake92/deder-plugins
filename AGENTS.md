...
- Run a single MUnit suite with the generated Deder classpath:
  ...
  ```
- After editing `deder-protobuf/src/main/resources/ProtobufPlugin.pkl`, regenerate the committed bindings:
  ```bash
...
- `deder-protobuf/src/main/scala/ba/sake/deder/protobuf/ProtobufPluginImpl.scala` is the plugin entrypoint. It evaluates the typed Pkl config from `ProtobufPlugin.pkl` and wires exactly three Deder tasks: `protobufSourceFiles`, `protobufGenerate`, and `protobufGenerateResources`.
...
- Treat `deder-protobuf/src/main/resources/ProtobufPlugin.pkl` as the source of truth for plugin configuration. The generated Java bindings under `deder-protobuf/src/main/java` and extra generated resources are committed and must be regenerated when the Pkl schema changes.
...
- New tests should follow the existing subsystem split under `deder-protobuf/src/test/scala/ba/sake/deder/protobuf/`: `config`, `sources`, `protoc`, and `tasks`. The suites are MUnit `FunSuite`s named after the runtime component they cover.
